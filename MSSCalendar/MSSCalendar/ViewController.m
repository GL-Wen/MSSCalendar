...
- (void)calendarClick:(UIButton *)btn
{
    ...
    cvc.beforeTodayCanTouch = false;// 今天之前的日期是否可以点击
    cvc.afterTodayCanTouch = true;// 今天之后的日期是否可以点击
    cvc.startDate = _startDate;// 选中开始时间
    cvc.endDate = _endDate;// 选中结束时间
    /*以下两个属性设为YES,计算中国农历非常耗性能（在5s加载15年以内的数据没有影响）*/
    cvc.showChineseHoliday = false;// 是否展示农历节日
    cvc.showChineseCalendar = false;// 是否展示农历
    cvc.showHolidayDifferentColor = false;// 节假日是否显示不同的颜色
    cvc.showAlertView = false;// 是否显示提示弹窗
    cvc.single = true;
    cvc.delegate = self;
    cvc.availableWeekDays = @[@(1),@(6),@(3)];
